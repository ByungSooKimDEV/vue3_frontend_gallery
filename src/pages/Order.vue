        ...
        <div class="container">
            <main>
                <div class="py-5 text-center">
                    <h2>주문하기</h2>
                    <p class="lead">
                        Below is an example form built entirely with Bootstrap’s form controls. Each required form group has a validation state that can be triggered by attempting to submit the form
...
export default {
    ...
    setup() {
        ...
        const submit = () => {
            const args = JSON.parse(JSON.stringify(state.form)); // state.form 만 해도 되지만 참조값의 연결성을 끊어주기 위해서 JSON.parse(JSON.stringify()) 를 붙여줌...
